...
class SellerBaseMerchandiseListView(ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user, __ = JWTAuthentication().authenticate(request)
        
        try:
            seller = User.objects.prefetch_related('seller').get(id=str(user)).seller
            self.queryset = BaseMerchandise.objects.filter(seller=seller)
...
class SellerMerchandiseCreateView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user, __ = JWTAuthentication().authenticate(request)
        seller = User.objects.prefetch_related('seller').get(id=str(user)).seller.id
...
class SellerOptionMerchandiseCreateView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not BaseMerchandise.objects.filter(seller=seller, common_code=common_code):
            return JsonResponse({'error': 'MERCHANDISE COMMON CODE ERROR'}, status=401)
        else:
            request.data['basemerchandise'] = int(str(BaseMerchandise.objects.get(seller=seller, common_code=common_code)))
...
class SellerBaseMerchandiseUpdateDeleteView(UpdateAPIView, DestroyAPIView):
    ...
    def get_object(self, queryset=None):
        common_code = self.kwargs.get('code')
        obj = BaseMerchandise.objects.get(common_code=common_code)
        return obj
    ...
    def patch(self, request, *args, **kwargs):
        ...
        user, __ = JWTAuthentication().authenticate(request)
        if str(self.get_object().seller) != str(user):
        ...
        try:
            ...
            common_code = self.get_object().common_code
...
class SellerOptionMerchandiseUpdateDeleteView(UpdateAPIView, DestroyAPIView):
    ...
    def get_object(self, queryset=None):
        full_code = self.kwargs.get('code')
        obj = Merchandise.objects.get(full_code=full_code)
        return obj
    ...
    def patch(self, request, *args, **kwargs):
        ...
        user, __ = JWTAuthentication().authenticate(request)
        if str(BaseMerchandise.objects.get(common_code=self.get_object().full_code[0:8]).seller) != str(user):
            return JsonResponse({'error': 'AUTHENTICATION ERROR'}, status=403)
